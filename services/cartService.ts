import { getAuthToken, getGuestToken, saveGuestToken } from '@/services/tokenStorage';
...
export const addToCart = async (productId: number) => {
    ...
    // 🔥 SAVE GUEST TOKEN IF RECEIVED
    if (response.data?.guest_token) {
        await saveGuestToken(response.data.guest_token);
    }
...
export const getCart = async () => {
    ...
    const response = await axios.get(
        'https://app.swimwell.co.in/api/get-cart',
        { headers }
    );
